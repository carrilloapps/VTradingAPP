...
const BankRatesScreen = () => {
  ...
  // Optimization: Defer search filter to avoid UI lag on typing
  const deferredQuery = useDeferredValue(searchQuery);
  const [_, startTransition] = useTransition();

  const fetchRates = useCallback(
    async (isRefresh = false, nextInfo = { page: 1 }) => {
      try {
  ...
  const filteredRates = useMemo(() => {
    ...
    return result;
  }, [bankRates, deferredQuery, sortType]);
  ...
  const renderItem = useCallback(
    ({ item }: { item: CurrencyRate }) => {
      ...
      const formatValue = (val?: number) =>
        val
          ? `${val.toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Bs`
          : 'N/A';
        ...
        >
          ...
          <View style={styles.bcvBottomRow}>
            <View style={styles.bcvPriceContainer}>
              <Text style={[styles.bcvPrice, { color: bcvTextColor }]}>
                {bcvRate
                  ? bcvRate.value.toLocaleString('es-VE', {
          ...
          <View style={styles.decorativeIcon}>
            <BolivarIcon
              ...
              size={160}
              style={styles.decorativeIconOpacity}
            />
          </View>
        </LinearGradient>
      </View>
