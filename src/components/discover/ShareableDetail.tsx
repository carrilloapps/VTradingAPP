...
const ShareableDetail: React.FC<ShareableDetailProps> = ({
  ...
  if (type === 'ARTICLE') {
    const topImageWrapperStyle = [
      ...
      { height: (isVertical ? '40%' : '50%') as DimensionValue },
    ];
    ...
    const pillTextStyle = [
      styles.pillText,
      { color: theme.colors.onBackground, fontSize: pillFontSize },
    ];
    ...
    const modernTitleStyle = [
      styles.modernTitle,
      {
        ...
        fontSize: isVertical ? 90 : 65,
        lineHeight: isVertical ? 100 : 75,
      },
      title.length > 40 && {
        fontSize: isVertical ? 75 : 55,
        lineHeight: (isVertical ? 85 : 65) as number,
      },
      title.length > 80 && {
        fontSize: isVertical ? 65 : 45,
        lineHeight: (isVertical ? 75 : 55) as number,
      },
    ];
    const descriptionStyle = [
      styles.modernDescription,
      {
        color: hexToRgba(theme.colors.onBackground, 0.85),
        fontSize: isVertical ? 38 : 28,
        lineHeight: isVertical ? 54 : 40,
      },
    ];
    const storeIconsStyle = (gap: number) => [styles.storeIcons, { gap }];
    const storeIconCircleStyle = (size: number) => [
    ...
    const downloadTextStyle = [
      styles.downloadText,
      { color: theme.colors.onBackground, fontSize: footerTextSize },
    ];
    const urlBadgeStyle = [
      styles.urlBadge,
      {
        backgroundColor: hexToRgba(theme.colors.onBackground, 0.1),
        paddingHorizontal: isVertical ? 40 : 32,
        paddingVertical: isVertical ? 20 : 16,
      },
    ];
    const urlTextStyle = [
      styles.urlText,
      { color: theme.colors.onBackground, fontSize: urlTextSize },
    ];
          ...
          <View style={containerStyle}>
            ...
            <View style={topImageWrapperStyle}>
              ...
              <LinearGradient
                colors={[
                  'transparent',
                  hexToRgba(theme.colors.background, 0.2),
  ...
  const renderHeroWithList = () => {
    ...
    const compactCardImageStyle = (size: number) => [
      styles.cardImage,
    ...
    const compactCardTitleStyle = [
      styles.cardTitle,
      {
        ...
        lineHeight: isVertical ? 46 : 40,
      },
    ];
    const listItemAvatarStyle = (size: number) => [
      styles.listItemAvatar,
      { width: size, height: size, borderRadius: size / 2 },
    ];
    const listItemAvatarPlaceholderStyle = (size: number) => [
      styles.listItemAvatarPlaceholder,
      {
        width: size,
        height: size,
        borderRadius: size / 2,
    ...
    const listItemAvatarLetterStyle = (fontSize: number) => [
      styles.listItemAvatarLetter,
      { fontSize, color: theme.colors.onPrimaryContainer },
    ];
    const listItemAuthorStyle = [
      styles.listItemAuthor,
      {
        color: hexToRgba(theme.colors.onBackground, 0.9),
    ...
    const cardDateTextStyle = [
      styles.cardDate,
      {
        color: hexToRgba(theme.colors.onBackground, 0.7),
      ...
      <View style={styles.listContainer}>
        ...
        <Surface style={heroCardStyle} elevation={5}>
          ...
          <View style={styles.heroContent}>
            ...
            <Text style={heroTitleTextStyle} numberOfLines={2}>
              {heroItem.title}
            </Text>
              ...
              <View key={index} style={compactCardStyle(compactItemHeight)}>
                {item.image && (
                  <FastImage
                    ...
                    style={compactCardImageStyle(compactItemImageSize)}
                    resizeMode={FastImage.resizeMode.cover}
                  />
                )}
                <View style={styles.cardContent}>
                  <Text style={compactCardTitleStyle} numberOfLines={2}>
                    {item.title}
                  </Text>
                  <View style={styles.listItemMeta}>
                    ...
                    {item.authorAvatar ? (
                      <FastImage
                        ...
                        style={listItemAvatarStyle(avatarSize)}
                      />
                    ) : (
                      <View style={listItemAvatarPlaceholderStyle(avatarSize)}>
                        <Text style={listItemAvatarLetterStyle(avatarFontSize)}>
                          {(item.author || 'V')[0]}
                        </Text>
                      </View>
                    )}
                    ...
                    {/* Author Name */}
                    <Text style={listItemAuthorStyle}>{item.author}</Text>

                    {/* Separator */}
                    <MaterialCommunityIcons
                      name="circle-small"
                      size={isVertical ? 32 : 24}
                      color={hexToRgba(theme.colors.onBackground, 0.5)}
                    />
  ...
  const topPillTextStyle = [
    styles.pillText,
    { color: theme.colors.onBackground, fontSize: pillFontSize },
  ];
  const topBrandLogoStyle = [
    styles.brandLogo,
    {
      tintColor: theme.colors.onBackground,
      width: logoWidth,
  ...
  const mainTitleStyleScaled = [
    ...
    title.length > 40 && {
      fontSize: isVertical ? 85 : 60,
      lineHeight: (isVertical ? 95 : 68) as number,
    },
    title.length > 80 && {
      fontSize: isVertical ? 75 : 50,
      lineHeight: (isVertical ? 85 : 58) as number,
    },
  ];

  const oldDescriptionStyle = [
    styles.description,
    {
      color: hexToRgba(theme.colors.onBackground, 0.8),
      fontSize: isVertical ? 42 : 32,
      lineHeight: isVertical ? 60 : 44,
    },
  ];
  const oldStoreIconsStyle = [styles.storeIcons, { gap: isVertical ? 24 : 16 }];
  const oldStoreIconCircleStyle = (size: number) => [
    styles.storeIconCircle,
    {
      backgroundColor: theme.colors.onBackground,
      width: size,
  ...
  const oldDownloadTextStyle = [
    styles.downloadText,
    { color: theme.colors.onBackground, fontSize: footerTextSize },
  ];
  const oldUrlBadgeStyle = [
    styles.urlBadge,
    {
      backgroundColor: hexToRgba(theme.colors.onBackground, 0.15),
      paddingHorizontal: isVertical ? 40 : 32,
      paddingVertical: isVertical ? 20 : 16,
    },
  ];
  const oldUrlTextStyle = [
    styles.urlText,
    { color: theme.colors.onBackground, fontSize: urlTextSize },
  ];
        ...
        <View style={containerStyle}>
          ...
          <View style={styles.contentContainer}>
            ...
            >
              ...
              {/* Middle Content (Card) */}
              <View style={styles.middleSection}>{renderHeroWithList()}</View>
            </View>
