...
const WidgetCard: React.FC<WidgetCardProps> = ({
  ...
  const getGradientColors = () => {
    ...
    return isWidgetDarkMode ? ['#212121', '#2C2C2C', '#212121'] : ['#FFFFFF', '#F2F4F6', '#FFFFFF'];
  };
    ...
    >
      ...
      <View style={styles.widgetHeader}>
        <View style={styles.widgetTitleRow}>
          ...
          />
          <Text style={[styles.widgetTitleText, { color: widgetTitleColor }]}>{widgetTitle}</Text>
        </View>
        <MaterialCommunityIcons name="refresh" size={18} color={refreshIconColor} />
      </View>
      ...
      {items.slice(0, 4).map((item, index) => {
        ...
        return (
          <View
            key={item.id}
            style={[isLast ? styles.rateRowNoBorder : styles.rateRow, rowBorderStyle]}
          >
            <View>
              <Text style={[styles.currencyLabel, { color: currencyLabelColor }]}>
                {item.label}
              </Text>
              <Text style={[styles.rateValue, { color: rateValueColor }]}>
                {item.value}{' '}
                <Text style={[styles.rateCurrency, { color: rateCurrencyColor }]}>
                  {item.currency}
                </Text>
              </Text>
            </View>
      ...
      {!showGraph && (
        <View style={styles.widgetFooter}>
          ...
          <Text style={[styles.footerTextBold, { color: footerTextColor }]}>BCV • Binance</Text>
        </View>
      )}
    </LinearGradient>
  );
