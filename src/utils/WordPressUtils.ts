 ...
 */
export const getCategoryImage = (cat: WordPressCategory): string | undefined => {
  ...
  if (cat.acf?.image) {
    ...
    if (typeof cat.acf.image === 'object' && cat.acf.image.url) return cat.acf.image.url;
  }

  if (cat.acf?.icon) {
    if (typeof cat.acf.icon === 'string') return cat.acf.icon;
    if (typeof cat.acf.icon === 'object' && cat.acf.icon.url) return cat.acf.icon.url;
  }
